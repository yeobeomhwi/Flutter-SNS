...
exports.notifyPostOwner = functions.firestore
    .document("posts/{postId}")
    .onUpdate(async (change, context) => {
      const beforeComments = change.before.data().comments || [];
      const afterComments = change.after.data().comments || [];

      // 새로 추가된 댓글 감지
      if (afterComments.length <= beforeComments.length) return null;
      const newComment = afterComments[afterComments.length - 1];

      const userId = change.after.data().userId; // 게시물 작성자의 userId
...
exports.notifyPostOwnerOnLike = functions.firestore
    .document("posts/{postId}")
    .onUpdate(async (change, context) => {
